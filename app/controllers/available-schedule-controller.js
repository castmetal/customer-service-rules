    ...
    .then(() => {
      ...
      if (!checkDate(start_date, end_date)) {
        const error = new Error(
          `end_date deve ser maior ou igual que start_date`
        );
...
exports.validate = method => {
  switch (method) {
    case 'getAvailableSchedules': {
      return [
        query('start_date', genericErrors.INVALID_DATE).exists(),
        query('start_date').custom(day => {
          if (day && moment(day, 'DD-MM-YYYY', true).isValid() === false) {
            return Promise.reject(genericErrors.INVALID_DATE);
          }
          return Promise.resolve(day);
        }),
        query('end_date', genericErrors.INVALID_DATE).exists(),
        query('end_date').custom(day => {
          if (day && moment(day, 'DD-MM-YYYY', true).isValid() === false) {
            return Promise.reject(genericErrors.INVALID_DATE);
          }
          return Promise.resolve(day);
